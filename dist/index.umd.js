...
let loadingCount = 0;
/* harmony default export */ var mixin = (function ({
  store,
  handleError
}) {
  return {
      ...
      $loading() {
        ...
        if (asyncMethods && asyncMethods.constructor) {
          ...
          funcNames.forEach(funcName => {
            curCompLoading[funcName] = store.state.loading.loading[`${compName}/${funcName}`];
          });
        }

        return { ...curCompLoading,
...
/* harmony default export */ var src_0 = ({
  install(Vue, {
    ...
    // 注册 store
    if (store) {
      store.registerModule('loading', src_store);
    }
